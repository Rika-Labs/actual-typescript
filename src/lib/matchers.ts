...
import { ActualDefaultError } from "../models/errors/actual-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ActualDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ActualDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ActualDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    switch (encoding) {
      case "json":
        ...
        try {
          ...
          return [ERR(new ResponseValidationError("Response JSON decoding failed", {
            cause,
            rawValue: body,
            rawMessage: "Response JSON decoding failed",
            request,
            response,
            body,
          })), raw];
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ActualDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
